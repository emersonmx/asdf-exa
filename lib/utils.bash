...
list_github_tags() {
  git ls-remote --tags --refs "$GH_REPO" |
    ...
    sed -E -e 's/^v//' -e '/^[0-9]+\.[0-9]+\.[0-9]+$/! d' # NOTE: You might want to adapt this sed to remove non-version strings from tags
}
...
install_version() {
  ...
  (
    ...
    mv -f "$install_path/bin/$tool_cmd-${platform}" "$tool_path"
    chmod +x "$tool_path"
